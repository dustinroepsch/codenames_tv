...
export default function ScoreBar({ board, currentTurn }: ScoreBarProps) {
  ...
  const redRevealed = board.filter((c) => c.color === "red" && c.revealed).length;
  const blueRevealed = board.filter((c) => c.color === "blue" && c.revealed).length;
  ...
  const blueTotal = blueRemaining > 0 ? blueRemaining + blueRevealed : blueRevealed;
  const redLeft = redTotal - redRevealed;
  const blueLeft = blueTotal - blueRevealed;

  return (
    <div className="score-bar">
      <div className={`score score-red ${currentTurn === "red" ? "active-turn" : ""}`}>
        <span className="score-label">RED</span>
        <span className="score-number">{redLeft}</span>
      </div>
      <div className={`score score-blue ${currentTurn === "blue" ? "active-turn" : ""}`}>
